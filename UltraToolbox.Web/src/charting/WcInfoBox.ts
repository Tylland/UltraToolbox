...
export class WcInfoBox extends WcFigure implements IExpandable {
    private settings: Settings = new Settings();
    public expanded: boolean = false;

    constructor(private start: number, private end: number, private height: number, private info: string) {
        super();
    }
    ...
    private drawGradient(renderer: IRenderer, rect: IRect, boxSettings: BoxSettings, labelSettings: LabelSettings): void {
        ...
        renderer.drawText(this.info, rect.getCenter(), labelSettings.font, labelSettings.fill, TextAlignment.CenterMiddle);
    }
    ...
    public draw(renderer: IRenderer, view: IWcView): void {
        ...
        if (this.expanded) {
            ...
            let textSize: IRect = view.device.measureText(this.info, this.settings.labelHighlight.font);
